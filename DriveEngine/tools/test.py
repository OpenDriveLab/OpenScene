...
def main():
    ...
    # caculate the model parameters and flops
    # model_parameters = 0
    # for name, para in model.named_parameters():
    #     if 'transformer.decoder' in name:
    #         continue
    #     if para.requires_grad:
    #         model_parameters = model_parameters + para.numel()
    #     # if 'transformer.decoder' in name \
    #     #         or 'bev_embedding' in name or 'voxel_embedding' in name \
    #     #         or 'transformer.pos' in name or 'transformer.can_bus' in name or 'transformer.voxel2bev' in name \
    #     #         or 'transformer.bev_voxel_transfers' in name or 'transformer.image_feature_map' in name:
    #     #         para.requires_grad = False
    
    # # sum(p.numel() for p in model.parameters() if p.requires_grad)
    # print('parameters count:', model_parameters)

    # occ_thresholds = [0.20, 0.25, 0.30]  # TODO modify the threshold to output occ
    occ_thresholds = [0.25]
    for index, occ_threshold in enumerate(occ_thresholds):
        ...
        if rank == 0:
            ...
            # save occupancy_results
            # if occupancy_results is not None and args.show_dir:
            #     occ_save_dir = os.path.join(args.show_dir, 'occ_pred')
            #     os.makedirs(occ_save_dir, exist_ok=True)
            #     save_interval = 1
            #     for index in range(len(occupancy_results)):
            #         if index % save_interval == 0:
            #             cur = occupancy_results[index]
            #             save_path = osp.join(occ_save_dir, '{:05d}.pkl'.format(index))
            #             torch.save(cur, save_path)
