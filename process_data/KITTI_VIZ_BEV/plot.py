...
# import yaml
import pandas as pd
from KITTI_VIZ_BEV.kitti_util import *
from pylab import *
...
def point_cloud_2_top(points,
                      res=0.05,
                      zres=0.3,
                      side_range=(-40., 40-0.05),  # left-most to right-most
                      fwd_range=(-40., 40.-0.05),  # back-most to forward-most
                      height_range=(0, 2.),  # bottom-most to upper-most
                      ):
    ...
    top = np.zeros([x_max+1, y_max+1, z_max+1], dtype=np.float32)
    ...
    for i, height in enumerate(np.arange(height_range[0], height_range[1], zres)):
        ...
        x_img = (-xi_points / res).astype(np.int32)  # x axis is -y in LIDAR
        y_img = (-yi_points / res).astype(np.int32)  # y axis is -x in LIDAR
...
def draw_point_cloud(ax, points, axes=[0, 1, 2], point_size=0.1, xlim3d=None, ylim3d=None, zlim3d=None):
    ...
    ax.scatter(*np.transpose(points[:, axes]), s=point_size, c=points[:, 3])
...
def read_detection(path):
    ...
    try:
        ...
        df.columns = ['type', 'truncated', 'occluded', 'alpha', 'bbox_left', 'bbox_top',
                    'bbox_right', 'bbox_bottom', 'height', 'width', 'length', 'pos_x', 'pos_y', 'pos_z', 'rot_y','score', 'No']
    # df.columns = ['type', 'truncated', 'occluded', 'alpha', 'bbox_left', 'bbox_top',
...
def plot_fig(boxes,ax,linestyle='--',colors='cyan'):
    for o in range(len(boxes)):
        ...
        x1, x2, y1, y2 = transform_to_img(x1, x2, y1, y2, res=0.1, side_range=(-80., 80-0.1), fwd_range=(-80., 80.-0.1))
        x3, x4, y3, y4 = transform_to_img(x3, x4, y3, y4, res=0.1, side_range=(-80., 80-0.1), fwd_range=(-80., 80.-0.1))
        ps=[]
...
def point_bev(points, boxes,names=None,fig_path='./1.png',use_default=False):
    ...
    top = point_cloud_2_top(points, res=0.1, zres=1.0, side_range=(-80., 80-0.1), fwd_range=(-80., 80.-0.1))
    top = np.array(top, dtype = np.float32)
    top_2 = np.zeros([1600,1600,4], dtype = np.float32)
    top_2[:,:,:3] = top[:,:,:]
    for i in range(1600):
        for j in range(1600):
            if (top[i,j,0]+top[i,j,1]+top[i,j,2]) == 0:
                top_2[i,j,3] = 0
            else:
                top_2[i,j,3] = 1
                top_2[i,j,0] = top[i,j,0]*0.7
                top_2[i,j,1] = top[i,j,1]*0.7
                top_2[i,j,2] = top[i,j,2]*0.7
    # print(top,top.shape)
    # ax.imshow(top_2[:,:,3], cmap='pink', aspect='equal')
    ax.imshow(top_2, cmap='hsv', aspect='equal')

    # plot_fig(df_r1, linestyle='-', color='red')
    if use_default == True:
        plot_fig(boxes,ax, linestyle='--', colors='green')
    else:
        color_map = {'vehicle':'green', 'bicycle': 'blue', 'pedestrian': 'red', 'traffic_cone': 'darkred', 'barrier': 'peru', 'czone_sign': 'pink', 'generic_object': 'black'}
        colors = [color_map[name] for name in names]
        plot_fig(boxes,ax, linestyle='--', colors=colors)
    plt.scatter(800, 800, c='red', marker='o', s=2)
    ...
    plt.show()
    dpi=500
    # output_dir = './data/'+sequence+'/bv2'
    # output_name = '%06d.png'%img_id
    # fig.savefig(os.path.join(output_dir, '{}'.format(output_name)), dpi=dpi, pad_inches = 0)
    fig.savefig(fig_path, dpi=dpi, pad_inches = 0)
    plt.close('all')
