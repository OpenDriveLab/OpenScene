...
class HybridFormerOccupancyHead(DETRHead):
    """Head of Detr3D.
    Args:
    ...
    def __init__(self,
        ...
        self.only_occ = only_occ  # only output occupancy
        self.only_det = only_det  # only output the detection result 
        self.det_dims = det_dims
        self.last_voxel_dims = last_voxel_dims 
        self.box_query_dims = box_query_dims
        self.use_fine_occ = use_fine_occ
        
        if self.det_dims is None:  # TODO 
            self.det_dims = kwargs.get('in_channels', 256)
        ...
        self.real_w = self.pc_range[3] - self.pc_range[0]  # x-direction
        self.real_h = self.pc_range[4] - self.pc_range[1]  # y-direction
        self.real_z = self.pc_range[5] - self.pc_range[2]  # z-direction
        ...
        self.occ_num = self.occ_xdim*self.occ_ydim*self.occ_zdim
        self.bev_num = self.bev_h*self.bev_w

        self.flow_gt_dimension = flow_gt_dimension  # 2D for nuscenes
        self.loss_occupancy = loss_occupancy
        self.loss_flow = loss_flow

        super(HybridFormerOccupancyHead, self).__init__(
            *args, transformer=transformer, **kwargs)
        self.code_weights = nn.Parameter(torch.tensor(
            self.code_weights, requires_grad=False), requires_grad=False)

        if self.loss_occupancy is not None:
            self.loss_occupancy = build_loss(loss_occupancy)
            self.predict_occupancy = True
        else:
            self.predict_occupancy = False
    ...
    def _init_layers(self):
        ...
        cls_branch = []
        for lay_index in range(self.num_reg_fcs):
            if lay_index == 0:
                cls_branch.append(Linear(self.embed_dims, self.det_dims))
            else:
                cls_branch.append(Linear(self.det_dims, self.det_dims))
            cls_branch.append(nn.LayerNorm(self.det_dims))
            cls_branch.append(nn.ReLU(inplace=True))
        cls_branch.append(Linear(self.det_dims, self.cls_out_channels))
        fc_cls = nn.Sequential(*cls_branch)

        reg_branch = []
        for lay_index in range(self.num_reg_fcs):
            if lay_index == 0:
                reg_branch.append(Linear(self.embed_dims, self.det_dims))
            else:
                reg_branch.append(Linear(self.det_dims, self.det_dims))
            reg_branch.append(nn.ReLU())
        reg_branch.append(Linear(self.det_dims, self.code_size))
        ...
        if not self.as_two_stage:
            ...
            self.query_embedding = nn.Embedding(self.num_query,
                                                self.box_query_dims * 2)

        if self.bev_z == self.occ_zdim or self.use_fine_occ:
            self.occ_proj = Linear(self.last_voxel_dims, self.occ_dims)  # the voxel size is consistent with the occupancy size
        else:
            self.occ_proj = Linear(self.bev_z*self.last_voxel_dims, self.occ_dims * self.occ_zdim)
        # occupancy branch
        if self.loss_occupancy is not None:
            occ_branch = []
    ...
    def init_weights(self):
        ...
        if self.loss_occupancy is not None and self.loss_occupancy.use_sigmoid:
            bias_init = bias_init_with_prob(0.01)
            nn.init.constant_(self.occ_branches[-1].bias, bias_init)
        if self.with_occupancy_flow:
            nn.init.constant_(self.flow_fc[-1], 0)
        # TODO initialize self.flow_branches
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None,  only_bev=False):
        ...
        if only_bev:  # only use encoder to obtain BEV features, TODO: refine the workaround
            ...
            block_features = self.transformer.get_voxel_features(
                    mlvl_feats,
                    bev_queries,
                    self.bev_z,
                    self.bev_h,
            ...
            bev_embed = block_features[-1]  # bev_embed: [bs, num_query, embed_dims]
            if prev_bevs is not None:  # TODO  to be designed if used
                bev_for_occ = torch.cat((*prev_bevs, bev_embed), dim=0)
            ...
            if self.use_fine_occ:
                bev_for_occ = bev_for_occ.view(bs*seq_len, self.bev_z, self.bev_h, self.bev_w, -1)
                bev_for_occ = bev_for_occ.permute(0, 4, 1, 2, 3)
                bev_for_occ = self.up_sample(bev_for_occ)  # (bs*seq_len, C, occ_z, occ_y, occ_x)  3D upsampling
                bev_for_occ = bev_for_occ.permute(0, 2, 3, 4, 1)
            ...
            block_features, hs, init_reference, inter_references = outputs
            # bev_embed: [bev_z*bev_h*bev_w, bs, embed_dims]
            # hs:  (num_dec_layers, num_box_query, bs, embed_dims)
            # init_reference: (bs, num_box_query, 3)  in (0, 1)
            # inter_references:  (num_dec_layers, bs, num_box_query, 3)  in (0, 1)
            ...
            bev_embed = block_features[-1]
            bev_embed = bev_embed.permute(1, 0, 2)  # [bs, bev_z*bev_h*bev_w, embed_dims]
            ...
            occ_pred = occ_pred.reshape(bs * seq_len, -1, self.occ_dims)
            outputs_occupancy = self.occ_branches(occ_pred)  # (bs*seq_len, occ_num, 16)
    ...
    def loss(self,
        ...
        gt_occupancy = (torch.ones((bs*seq_len, self.occ_num), dtype=torch.long)*self.occupancy_classes).to(device)
        for sample_id in range(len(temporal_gt_bboxes_list)):
            for frame_id in range(seq_len):
                occ_gt = occ_gts[sample_id][frame_id]
                gt_occupancy[sample_id*seq_len+frame_id][[occ_gt[:, 0]]] = occ_gt[:, 1]

        if flow_preds is not None:
            gt_flow = torch.zeros((bs*seq_len, self.occ_num, 2)).to(device)
            for sample_id in range(len(temporal_gt_bboxes_list)):
                for frame_id in range(seq_len):
                    occ_gt = occ_gts[sample_id][frame_id]
        ...
        gt_bboxes_list = [torch.cat(
            ...
            dim=1).to(device) for gt_bboxes in gt_bboxes_list]   # list(tensor), tensor.shape: (num_box, 9)
        ...
        occupancy_preds = occupancy_preds.view(-1, self.occupancy_classes)  # (bs*seq_len*occ_zdim*occ_ydim*occ_xdim, 11)
        ...
        #if self._training_iter_ % 1000 == 0 and get_dist_info()[0] == 0:
        #    torch.save(occupancy_preds, f'pred_basev0__{self._training_iter_}.ckpt')
        #    torch.save(gt_occupancy, f'gt_occ_basev0__{self._training_iter_}.ckpt')
        #self._training_iter_ += 1
    ...
    def loss_only_occupancy(self,
                        ...
                        gt_bboxes_ignore=None,
                        img_metas=None):
        """"Loss function for the task of occupancy prediction
        """
    ...
    def get_occupancy_prediction(self, occ_results, occ_threshold=0.25):
        ...
        occ_index, = torch.where(occ_class < self.occupancy_classes)  # torch.int64
