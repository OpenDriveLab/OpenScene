...
class CustomNuDataset(NuScenesDataset):
    r"""NuScenes Dataset.
    ...
    def load_annotations(self, ann_file):
        ...
        """
        # data = mmcv.load('/cpfs01/user/liyang/zhouys/Datasets/nuPlan/occupancy/nuscenes_infos_temporal_val.pkl')
    ...
    def get_data_info(self, index):
        ...
        input_dict = dict(
            ...
            # gt_bboxes_3d=info['gt_boxes_st'],
            # gt_labels_3d=info['gt_names']
        )

        # input_dict['gt_names'] = np.array(['car' for name in info['gt_names']])
        # input_dict['gt_names'] = input_dict['gt_labels_3d']
    ...
    def evaluate_occ_iou(self, occupancy_results, flow_results, show_dir=None, 
        ...
        self.eval_metrics = SSCMetrics(self.occupancy_classes, 
                                       ...
                                       occ_type=self.occ_type)  # 先累加最后计算指标
