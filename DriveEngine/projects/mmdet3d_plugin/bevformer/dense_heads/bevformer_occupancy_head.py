...
class BEVFormerOccupancyHead(DETRHead):
    ...
    def loss(self,
        ...
        else:  # calculate occ gt on line
            gt_occupancy = point_coords[0][0].new_full((bs*seq_len, self.occ_zdim, self.occ_ydim, self.occ_xdim),
                                              self.occupancy_classes,
                                              dtype=torch.long)  # TODO  the order of z, y, x
            occupy_inds = [[pc.long().split(1, dim=-1)[1:] for pc in pc_sample] for pc_sample in point_coords]  # order: z,y,x dim??
            for sample_id, gt_bboxes in enumerate(temporal_gt_bboxes_list):
                coords_sample = point_coords[sample_id]
                for frame_id in range(seq_len):
                    coords = coords_sample[frame_id]  # (num_points, 4)  _, index_z, index_y, index_x
                    point_x = (coords[:, 3:4] + 0.5) / self.occ_xdim * (self.point_cloud_range[3] - self.point_cloud_range[0]) + self.point_cloud_range[0]
                    point_y = (coords[:, 2:3] + 0.5) / self.occ_ydim * (self.point_cloud_range[4] - self.point_cloud_range[1]) + self.point_cloud_range[1]
                    point_z = (coords[:, 1:2] + 0.5) / self.occ_zdim * (self.point_cloud_range[5] - self.point_cloud_range[2]) + self.point_cloud_range[2]
                    point_xyz = torch.cat((point_x, point_y, point_z), dim=-1).unsqueeze(0)  # (1, num_points, 3)
                    _gt_bboxes = gt_bboxes[frame_id].clone().tensor.to(device)[:, :7]

                    _gt_bboxes[:, 3:6] += 0.5  # increase the box size
                    _gt_bboxes = _gt_bboxes[:, [0, 1, 2, 4, 3, 5, 6]]
                    _gt_bboxes[:, -1] = -(_gt_bboxes[:, -1]+np.pi/2)

                    in_box = points_in_boxes_part(point_xyz, _gt_bboxes.unsqueeze(0))[0]  # shape: (num_points) value in [-1, _gt_bboxes.size(0))
                    in_box[in_box == -1] = _gt_bboxes.size(0)   # in_box == -1: this point is in the background
                    _gt_labels = torch.cat((temporal_gt_labels_list[sample_id][frame_id], torch.ones_like(temporal_gt_labels_list[sample_id][frame_id])[:1] * self.occupancy_classes-1))
                    # _gt_labels.shape: (num_gt_box+1)  _gt_labels[-1] = 10
                    point_label = _gt_labels[in_box.long()]  # value in [0,..., 10], 10 means background
                    occupy_inds = [[[_i.squeeze(-1) for _i in inds_temporal] for inds_temporal in inds_sample] for inds_sample in occupy_inds]
                    gt_occupancy[sample_id*seq_len+frame_id][occupy_inds[sample_id][frame_id]] = point_label  # TODO oder : z-y-x
                    # for gt_occupancy, 0-9 -> object, 10 -> background, 11 -> empty region
    ...
    def loss_semantic_kitti(self,
        ...
        if self.occ_loss_type == 'focal_loss':
            ...
            # loss_occupancy = self.loss_occupancy(occupancy_preds, gt_occupancy, weight=class_weights, ignore_index=255, reduction="mean")
    ...
    def get_occupancy_prediction(self, occ_results, occ_threshold=0.25):
        ...
        if self.occ_loss_type == 'focal_loss':
            ...
            occupancy_preds = occupancy_preds.reshape(-1, self.occupancy_classes)  # self.occupancy_classes = 20
