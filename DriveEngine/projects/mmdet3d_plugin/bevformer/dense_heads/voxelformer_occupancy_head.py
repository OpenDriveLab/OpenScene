...
class VoxelFormerOccupancyHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.use_fine_occ = use_fine_occ  # use finer occupancy with size = 0.25
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None,  only_bev=False):
        ...
        voxel_pos = self.positional_encoding(voxel_mask).to(dtype)  # (bs, embed_dims, d, h, w)

        if isinstance(prev_bev, list) or isinstance(prev_bev, tuple):
            prev_bevs = [f for f in prev_bev]
            prev_bev = prev_bev[-1]
