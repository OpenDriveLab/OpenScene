...
def split_scenes(scenes):
    ...
    return scenes[start:end], start+log_start
...
def create_nuplan_info(nuplandb_wrapper, test, max_sweeps=1, sweep_interval=1, sample_interval=10, out_dir='./', occ_resolution='normal', save_flow_info=True, filter_instance=True, use_cuda=False, is_nuplan=True, scene_process_type='reprocess', set_background_label=False):
    ...
    file_names = file_names[log_start:log_start+log_length]
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='process data for nuplan dataset')
    # parser.add_argument('config', help='train config file path')
    parser.add_argument('--data_root', help='NUPLAN_DATA_ROOT')
    parser.add_argument('--data_split', help='trainval/test/mini')
    parser.add_argument('--occ_res', help='occ_resolution')
    parser.add_argument('--out_dir', help='path of the output')
    parser.add_argument('--log_start', help='log start point for dlc')
    parser.add_argument('--log_length', help='log length for dlc')
    parser.add_argument('--num_thread', help='number of threads')
...
if __name__ == '__main__':
    ...
    NUPLAN_DATA_ROOT = args.data_root

    NUPLAN_DB_FILES = f"{NUPLAN_DATA_ROOT}/splits/"+args.data_split
    ...
    NUPLAN_MAPS_ROOT = NUPLAN_DATA_ROOT.replace('nuplan-v1.1', 'maps')
    ...
    global thread_num, log_start, log_length
    thread_num, log_start, log_length = int(args.num_thread), int(args.log_start), int(args.log_length)
