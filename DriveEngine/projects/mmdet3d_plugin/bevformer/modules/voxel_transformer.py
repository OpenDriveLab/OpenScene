...
class VoxelPerceptionTransformer(BaseModule):
    ...
    def init_layers(self):
        ...
        if self.decoder_on_bev and self.voxel_2_bev_type == 'mlp':
        ...
        if prev_bev is not None:  # (bs, num_query, embed_dims)
            ...
            if self.rotate_prev_bev:  # revise for 3D feature map
                for i in range(bs):
                    ...
                    tmp_prev_bev = prev_bev[:, i].reshape(bev_z, bev_h, bev_w, -1).permute(3, 0, 1, 2)  # (embed_dims, bev_z, bev_h, bev_w)
                    tmp_prev_bev = rotate(tmp_prev_bev, rotation_angle, center=self.rotate_center)
                    tmp_prev_bev = tmp_prev_bev.permute(1, 2, 3, 0).reshape(bev_z * bev_h * bev_w, 1, -1)
